...
(* the following function is purely for Harlequin MLWorks: it's function
 * is to remove all leading require statements from a string consisting
 * of the user declarations of an ml-yacc specification.  These requires
 * are put at the beginning of the output, whereas the other user decs
 * are put within a structure.
 *)
    ...
    *)
    ...
    val printEC = fn (keyword : term list,
	 ...
	 val printErrValues = fn (l : (term * string) list) =>
	    (sayln "val errtermvalue= fn x=>";  (* the eta expansion is for
                                                   value poly. *)
...
let val printAbsynRule = Absyn.printRule(say,sayln)
    ...
    in printAbsynRule (RULE(pat,body))
	  ...
	  val prRules = fn () =>
	   ...
	   sayln "val actions = fn x => "; (* eta expand for val. poly*)
