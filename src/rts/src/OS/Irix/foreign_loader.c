 ...
 *
 *  Implementation  (IRIX/MIPS)
 *  --------------
 *  This implementation provides low-level ML FI support, via use of
 *  the dynamic linking utilities (dlopen() etc.) and ELF object code
 *  format.  It is strongly based on the implementation given for
 *  Solaris.
 *
 *
 *  Revision Log
 ...
 *  Revision 1.3  1997/02/28 09:57:56  stephenb
 *  [Bug #1932]
 ...
 *  numerous assertion statements.
 *
 * Revision 1.1  1996/02/26  14:19:34  brianm
 * new unit
 * Moved from architecture specific location.
 *
 * Revision 1.5  1996/02/14  16:50:24  brianm
 * Adding support for detecting endianness of underlying machine.
 *
 * Revision 1.4  1995/05/17  15:58:05  brianm
 * Updating foreign_loader to implement ML FI on MIPS.
 *
 * Revision 1.3  1995/03/24  12:20:59  brianm
 * Adding prototypes due to modification of header file.
 * Simplified code to yield `unimplemented' exception.
 *
 * Revision 1.2  1995/03/01  17:19:27  brianm
 * Minor corrections.
 *
 * Revision 1.1  1995/03/01  10:59:12  brianm
 * new unit
...
#include <libelf.h>     /* ELF libraries - Irix               */
...
static mlval next_symtab_entry(mlval argument)
{
  ...
  */
  ...
  for(i=entry;i<entries;i++){
     ...
     st_shndx  =  appmask(symentry -> st_shndx, 0xf);
