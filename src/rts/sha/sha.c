...
/* compute the SHA digest of a string - daveb 03/11/96 */
void sha_string(SHA_INFO *sha_info, char* str, int len)
{
    BYTE data[BLOCK_SIZE];
    char *cp;
...
char *sha_sprint(SHA_INFO *sha_info)
{
    char* buffer;

    buffer = malloc(SHA_DIGESTSIZE * 2 + 1);
    if (buffer == NULL) {
      fprintf(stderr, "malloc failed\n");
      fflush(stderr);
      exit(1); /* No point in continuing and going bus error */
    }
    sprintf(buffer, "%08lx%08lx%08lx%08lx%08lx\n",
	sha_info->digest[0], sha_info->digest[1], sha_info->digest[2],
	sha_info->digest[3], sha_info->digest[4]);
    buffer[SHA_DIGESTSIZE * 2] = '\0';
    return(buffer);
}
