 ...
 *
 * $Log: base.sml,v $
 * Revision 1.2  1998/08/05 17:33:52  jont
 * Automatic checkin:
   ...
   struct
	...
	struct
	 ...
	 {is_keyword : term -> bool,
      ...
      local 
	(* val print = fn s => output(std_out s) --- in MLWorks toplevel *)
	 val println = fn s => (print s; print "\n")
	 val showState = fn (STATE s) => "STATE " ^ (int_to_string s)
      in
             ...
             (println "Parse: state stack:";
              ...
              case action
                of SHIFT state => println ("SHIFT " ^ (showState state))
                 | REDUCE i => println ("REDUCE " ^ (int_to_string i))
                 | ERROR => println "ERROR"
...
val mkFixError = fn ({is_keyword,preferred_subst,terms,errtermvalue,
  ...
  let fun FixError(lexv as (TOKEN (term,value as (_,leftPos,_)),_),
	...
	val (_,numStateList) = fold (fn (a,(num,r)) => (num+1,(a,num)::r))
				stateList (0,nil)
...
(* foldStateList: accumulates results while scanning numStateList *)
	...
	fun foldStateList f start = fold f numStateList start

(* foldLexVList: accumulates results while scanning lexVList *)

	fun foldLexVList f start = fold f lexVList start
